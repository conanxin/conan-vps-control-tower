...
1. `Hero 总览`：总体状态、一句话摘要和关键 chips。
2. `快速操作`：进入 3X-UI 面板、查看诊断、查看最近事件。
3. `代理链路`：VPS -> 代理核心 -> 面板可达 -> 端口开放。
4. `核心状态`：VPS、代理核心、3X-UI 面板、端口四张卡片。
5. `管理入口：3X-UI 面板`：脱敏公开入口和进入面板按钮。
6. `次级信息区`：可选检查、告警通知、健康历史、最近事件、诊断详情。

默认情况下，页面优先展示当前状态和行动入口；详细历史、事件和命令放在折叠区，避免首屏信息过载。
