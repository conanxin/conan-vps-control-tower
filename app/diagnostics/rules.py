...
def panel_down_but_proxy_may_work(checks: dict[str, CheckResult]) -> DiagnosticItem | None:
    ...
    return make_item(
        ...
        "Management panel access may be affected, while existing proxy traffic may still be usable. 当前代理核心和端口仍正常时，优先不要重启代理。",
        "Panel process, panel port, protocol, or configured hidden path may be unhealthy.",
        "Check panel HTTPS response and local panel port listening state. 需要修改配置时，可通过管理入口进入 3X-UI 面板。",
