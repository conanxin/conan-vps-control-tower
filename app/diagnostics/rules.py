...
def panel_down_but_proxy_may_work(checks: dict[str, CheckResult]) -> DiagnosticItem | None:
    ...
    return make_item(
        ...
        panel.message,
        "Management panel access may be affected, while existing proxy traffic may still be usable.",
        "Panel process, panel port, or local HTTP access may be unhealthy.",
        "Check panel HTTP response and local panel port listening state.",
        [commands.check_panel_http_command, commands.check_panel_port_command],
